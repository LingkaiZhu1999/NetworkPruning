...
Prune_GradfromW_Add_Grad, Prune_rankW_add_rankGrad_Add_Grad, Prune_rankW_add_rankGrad_Add_Random, Prune_rankW_mul_rankGrad_Add_Random, Prune_rankW_mul_rankGrad_Add_Grad, Prune_rankW_add_rankGrad, Prune_WfromGrad_Add_Grad, Prune_WfromGrad_Add_Grad_and_Random, prune_W_Add_Activation, Prune_W_Add_Activation
...
class Proposed_prune():
    ...
    def train(self, model, train_loader, optimizer, criterion, train_epoch, args):
        ...
        for batch_idx, (imgs, targets) in enumerate(train_loader):
            ...
            elif self.args.add_criterion == "activation":
                for name, module in self.model.named_modules():
                    if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
                        self.visualization.update({module: self.visualization.get(module).mean(dim=0)})
                pre_module = None
                i = 0
                for module, _ in self.parameters_to_prune:
                    if i > 0:
                        sorted_pre, indices_pre = F.relu(self.visualization.get(pre_module)).sort(descending=False)
                    else:
                        sorted_pre, indices_pre = F.relu(torch.flatten(imgs, 1).mean(dim=0)).sort(descending=False)
                    sorted_aft, indices_aft = F.relu(self.visualization.get(module)).sort(descending=False)
                    self.importance_scores_add.update({(module, 'weight'): indices_aft.unsqueeze(dim=1).multiply(indices_pre.unsqueeze(dim=0))})
                    # print(indices_aft.unsqueeze(dim=1).multiply(indices_pre.unsqueeze(dim=0)))
                    # print(indices_aft.unsqueeze(dim=1), indices_pre.unsqueeze(dim=0))
                    # print('-----------------------------------------------')
                    pre_module = module
                    i += 1
        ...
        if train_iter % self.args.update_interval == 0:
            if self.args.add_criterion == "random":
                ...
                if self.args.prune_criterion == "|grad|":
                    ...
                    add_ratio_random = self.add_rate_decay.get_dr() * self.prune_rate_decay.get_dr() * remain
                    add_ratio_grad = (1 - self.add_rate_decay.get_dr()) * self.prune_rate_decay.get_dr() * remain
                    global_unstructure_double_importance_scores.global_unstructured_with_different_criteria(self.parameters_to_prune, pruning_method=Prune_WfromGrad_Add_Grad_and_Random, amount_prune=prune_ratio, amount_add_grad=add_ratio_grad, amount_add_random=add_ratio_random, importance_scores_prune=self.importance_scores_prune, importance_scores_add=self.importance_scores_add)
            elif self.args.add_criterion == "activation":
                for name, module in self.model.named_modules():
                    if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
                        prune.remove(module, 'weight')
                ...
                #     # print(self.importance_scores_add.get((module, name)))
                #     if self.importance_scores_add.get((module, name)) == None:
                #         print('yes it is none')
                if self.args.prune_criterion == "|w|":
                    global_unstructure_double_importance_scores.global_unstructured_with_different_criteria(self.parameters_to_prune, pruning_method=Prune_W_Add_Activation, amount_prune=prune_ratio+add_ratio, amount_add=add_ratio, importance_scores_prune=self.importance_scores_prune, importance_scores_add=self.importance_scores_add)
                # if self.args.prune_criterion == "|w|":
                #     for name, module in self.model.named_modules():
                #         prune_W_Add_Activation(module, amount_prune=to_prune_t, amount_add=to_add_t, importance_scores_prune=self.importance_scores_prune, importance_scores_add=self.importance_scores_add)
